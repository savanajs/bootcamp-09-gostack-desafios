...
export default function QuestionList({ navigation }) {
  ...
  const helps = useSelector(state => state.help.helps.map(item => ({
    ...item,
  ...
  return (
    ...
    <View>
      {(!loading) ? (
        <Container>
          <TouchableOpacity onPress={() => navigation.navigate('QuestionForm')}>
                ...
                <CustomCard>
                  <CustomCardHeader>
                    <CustomCardHeaderTextLeft>
                      {(!item.answer) ? (
                        <Text>
                          <Icon name="check-circle" size={11} color="rgba(0,0,0,.6)" />
                          {' '}
                      Sem respostas
                        </Text>
                      ) : (
